...
namespace {
    ...
    double
    standard_deviation(std::vector<double>::iterator first, std::vector<double>::iterator last) {
    ...
    namespace Benchmark {
        namespace Detail {
            ...
            bootstrap_analysis analyse_samples(double confidence_level, int n_resamples,
                ...
                double outlier_variance = Detail::outlier_variance(mean_estimate, stddev_estimate,
                                                                   n);
...
namespace Catch {
    ...
    bool Approx::equalityComparisonImpl(const double other) const {
        ...
        return marginComparison(m_value, other, m_margin)
               || marginComparison(m_value, other, m_epsilon * (m_scale + std::fabs(
...
namespace Catch {
    ...
    Config::getSectionsToRun() const { return m_data.sectionsToRun; }
...
namespace Catch {
    ...
    namespace {
        ...
        createReporter(std::string const &reporterName, IConfig const* config) {
            auto reporter = Catch::getRegistryHub().getReporterRegistry().create(reporterName,
                                                                                 config);
...
namespace Catch {
    ...
    TestSpec::Matches TestSpec::matchesByFilter(std::vector<TestCaseHandle> const &testCases,
...
namespace Catch {
    ...
    void
    IStreamingReporter::listTests(std::vector<TestCaseHandle> const &tests, IConfig const &config) {
        ...
        for (auto const &test : tests) {
            ...
            if (config.verbosity() >= Verbosity::High) {
                Catch::cout()
                        << TextFlow::Column(Catch::Detail::stringify(testCaseInfo.lineInfo)).indent(
                                4) << std::endl;
            }
...
namespace Catch {
    namespace {
        ...
        void listTags(IStreamingReporter &reporter, IConfig const &config) {
            auto const &testSpec = config.testSpec();
            std::vector<TestCaseHandle> matchedTestCases = filterTests(
                    getAllTestCasesSorted(config), testSpec, config);
...
namespace Catch {
    ...
    IStreamingReporterPtr
    ReporterRegistry::create(std::string const &name, IConfig const* config) const {
...
namespace Catch {
    ...
    void RunContext::sectionEnded(SectionEndInfo const &endInfo) {
        ...
        m_reporter->sectionEnded(
                SectionStats(endInfo.sectionInfo, assertions, endInfo.durationInSeconds,
                             missingAssertions));
        m_messages.clear();
        m_messageScopes.clear();
    }
    ...
    namespace Detail {
        namespace {
            ...
            private:
                ...
                int sync() override {
                    if (pbase() != pptr()) {
                        m_writer(std::string(pbase(), static_cast<std::string::size_type>( pptr() -
                                                                                           pbase())));
                        setp(pbase(), epptr());
                    }
                    return 0;
...
namespace Catch {
    ...
    bool
    matchTest(TestCaseHandle const &testCase, TestSpec const &testSpec, IConfig const &config) {
    ...
    void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const &functions) {
        ...
        for (auto const &function : functions) {
            ...
            CATCH_ENFORCE(prev.second,
                          "error: TEST_CASE( \"" << function.getTestCaseInfo().name
                                                 << "\" ) already defined.\n"
                                                 << "\tFirst seen at "
...
namespace Catch {
    ...
    TestSpecParser::TestSpecParser(ITagAliasRegistry const &tagAliases) : m_tagAliases(
            &tagAliases) {}
...
namespace Catch {
    ...
    namespace {
        ...
        bool shouldNewline(XmlFormatting fmt) {
            return !!(static_cast<std::underlying_type_t<XmlFormatting>>(fmt &
...
namespace Catch {
    namespace Matchers {
        ...
        StringEqualsMatcher::StringEqualsMatcher(CasedString const &comparator) : StringMatcherBase(
                "equals", comparator) {}
        ...
        StartsWithMatcher::StartsWithMatcher(CasedString const &comparator) : StringMatcherBase(
        ...
        EndsWithMatcher::EndsWithMatcher(CasedString const &comparator) : StringMatcherBase(
        ...
        RegexMatcher::RegexMatcher(std::string regex, CaseSensitive caseSensitivity) : m_regex(
...
namespace Catch {
    ...
    void
    ListeningReporter::listTests(std::vector<TestCaseHandle> const &tests, IConfig const &config) {
        for (auto const &listener : m_listeners) {
...
namespace Catch {
    ...
    void
    SonarQubeReporter::writeSection(std::string const &rootName, SectionNode const &sectionNode,
                                    bool okToFail) {
        std::string name = trim(sectionNode.stats.sectionInfo.name);
        if (!rootName.empty())
            name = rootName + '/' + name;
    ...
    namespace {
        ...
        public:
            ...
            TapAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
                    ...
                    : stream(_stream), result(_stats.assertionResult),
                      messages(_stats.infoMessages), itMessage(_stats.infoMessages.begin()),
                      printInfoMessages(true), counter(_counter) {}
...
namespace Catch {
    ...
    namespace {
        ...
        // subsequent lines
        void
        printHeaderString(std::ostream &os, std::string const &_string, std::size_t indent = 0) {
            std::size_t i = _string.find(": ");
            if (i != std::string::npos)
                i += 2;
            else
